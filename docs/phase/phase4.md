...
| 단계 | 고민/목표                                        | 검토 대상             | 판단 기준                  | 결과                   |
| -- | -------------------------------------------- | ----------------- | ---------------------- | -------------------- |
| 1  | 트래픽 급증 시 요청을 바로 거절하지 않고 **대기 → 처리 전환 구조 필요** | Queue 구조 필요       | 순차 처리 가능 여부            | Queue 구조 도입 결정       |
| 2  | Redis 기반으로 Queue 구현                          | List              | FIFO 처리 가능             | 시간 기반 제어 불가 → 탈락     |
| 3  | 요청 중복 방지 + 관리                                | Set               | 중복 제거                  | 정렬/시간 개념 없음 → 탈락     |
| 4  | Rate Limit도 함께 해결 필요                         | String (Counter)  | 구현 단순                  | Fixed Window 문제 → 탈락 |
| 5  | **시간 기반 + Queue + Rate Limit 모두 만족 필요**      | Sorted Set (ZSET) | 정렬, range 조회, 시간 기반 삭제 | ✅ 최종 선택              |
